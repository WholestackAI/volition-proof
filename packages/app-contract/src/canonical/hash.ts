 ...
 * Isomorphic SHA-256.
 *
 * The contract layer runs in three places — Next.js server routes, the browser
 * (live semantic diff while editing), and the Zeta CLI — and all three must
 * agree on `contractHash` byte for byte. `node:crypto` is unavailable in the
 * first of those, and `crypto.subtle` is async, so the digest is implemented
 * here. Matches `sha256(utf8(input))` exactly; verified against known vectors
 * in `hash.test.ts`.
 */
...
/** Short stable digest used inside clause ids. Collision-resistant enough at 10 hex chars. */
